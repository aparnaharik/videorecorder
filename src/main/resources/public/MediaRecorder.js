...
function showLogin() {
  ...
  videosContainer.style.display = 'none';  // Hide the lobby
}

function showLobby(facebookID, name) {
  console.log('showLobby -> ' + facebookID, name);
  ...
  videosContainer.style.display = 'block';  // Show the lobby
}
...
function createMediaTable() {
  ...
  xhr.open('GET', '/userMedias/search/findByUserName?userName=' + userid, true);
  xhr.send(null);
  xhr.onreadystatechange=function() {
      if (xhr.readyState==4 && xhr.status == 200) {
        var respData = JSON.parse(xhr.responseText);
          // since we are using jQuery, you don't need to parse response
          drawTable(respData._embedded.userMedias);
      }
  }
}
...
function drawTable(data) {
  ...
  for (var i = 0; i < data.length; i++) {
      drawRow(data[i], new_tbody);
  }
  $("#personDataTable").replaceChild(new_tbody, $("#personDataTable").tbodies[0])

}

function drawRow(rowData, new_tbody) {
    var row = $("<tr />")
    new_tbody.append(row); //this will append tr element to table... keep its reference for a while since we will add cels into it
    row.append($("<td>" + rowData.videoName + "</td>"));
    row.append($("<td>" + rowData.created + "</td>"));
    row.append($("<td><a href=\"/video/" + rowData.videoId + "\">Play</a></td>"));
}
...
function MediaStreamRecorder(mediaStream) {
    ...
    this.save = function(file, fileName) {
        if (!file) {
            ...
            var myFile = blobToFile(bigBlob, "video.webm");
            formData.append('uploadedFile',myFile);
            ...
            xhr.onreadystatechange=function() {
                  ...
                  video_id = this.responseText;
                  var jsonData = {};
                  jsonData["userName"] = userid;

                  jsonData["videoName"] = document.getElementById('video-name').value;

                  jsonData["videoId"] = video_id;

                  var xhr1 = new XMLHttpRequest();
                  xhr1.open('POST', '/userMedias', true);
